...
def train_C_to_Y(args,XtoC_model=None):
    ...
    if args.ckpt:
        #train checkpointed model
        train_data = CUB_CtoY_dataset(mode='cktp',config_dict=args.CUB_dataloader,transform=transform, model=XtoC_model) #If XtoC model is provided, use it to generate the concepts
        train_loader = DataLoader(train_data, batch_size=args.batch_size, shuffle=True, num_workers=4)
        val_loader = None
    else:
